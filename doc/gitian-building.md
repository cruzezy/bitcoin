...
*Setup instructions for a Gitian build of Netgold using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Netgold
...
- [Building Netgold](#building-netgold)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-netgold-host/README.md).
...
Clone the git repositories for netgold and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/netgold/netgold
```
...
in the netgold repository under 'Fetch and build inputs' to install sources which require
...
To build Netgold (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the netgold repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/netgold/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/netgold/netgold
...
For example:
```bash
URL=https://github.com/laanwj/netgold.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit netgold=${COMMIT} --url netgold=${URL} ../netgold/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit netgold=${COMMIT} --url netgold=${URL} ../netgold/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit netgold=${COMMIT} --url netgold=${URL} ../netgold/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the netgold git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../netgold/contrib/gitian-descriptors/*|sort|uniq )
...
NTGPATH=/some/root/path/netgold.git
SIGPATH=/some/root/path/netgold-detached-sigs.git

./bin/gbuild --url netgold=${NTGPATH},signature=${SIGPATH} ../netgold/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/netgold-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/netgold-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/netgold-osx-build.assert
```
...
[netgold/gitian.sigs](https://github.com/netgold/gitian.sigs/) repository, or if that's not possible create a pull
